...
class Dpt:
    ...
    def _allocate_buffers(self, context):
        ...
        # Iterate over the I/O tensors using their index and name
        for i in range(self._engine.num_io_tensors):
            tensor_name = self._engine.get_tensor_name(i)
            
            # Determine if it's an input or output tensor
            is_input = self._engine.get_tensor_mode(tensor_name) == trt.TensorIOMode.INPUT
            
            # Get the tensor's shape (including the batch dimension)
            tensor_shape = self._engine.get_tensor_shape(tensor_name)
            
            # Calculate total size (trt.volume calculates product of shape elements)
            # Use self._batch_size only if you are explicitly handling a fixed/max batch size.
            # For static engines, the batch size is part of the shape, so we use the tensor volume.
            ...
            # Append the device buffer to device bindings (required for execute_async_v2).
    ...
    def trt_infer(self, data):
        ...
        # Assuming only one input tensor for simplicity, adjust if more inputs exist
        np.copyto(self._input[0].host, data.ravel()) 

        # Push to device
        self._device_ctx.push()

        # --- NEW STEP: Explicitly set tensor addresses for execute_async_v3 ---
        for i, inp in enumerate(self._input):
            tensor_name = self._engine.get_tensor_name(i)
            # Map the device pointer to the input tensor name
            self._context.set_tensor_address(tensor_name, inp.device)
            # Transfer input data to the GPU (Host to Device)
            cuda.memcpy_htod_async(inp.device, inp.host, self._stream)

        # Assuming output tensors follow inputs (as they usually do in bindings)
        # Iterate over output tensors and set their addresses
        num_inputs = len(self._input)
        for i, out in enumerate(self._output):
            # Get the output tensor name (index i + number of inputs)
            tensor_name = self._engine.get_tensor_name(i + num_inputs) 
            # Map the device pointer to the output tensor name
            self._context.set_tensor_address(tensor_name, out.device)

        # Run inference. (Using v3, which relies on the set addresses)
        self._context.execute_async_v3(
            stream_handle=self._stream.handle
        )
        # --------------------------------------------------------------------

        # Transfer predictions back from the GPU. (Device to Host)
        [
    ...
    def run(
        ...
        # NOTE: If you need the visualization for saving, you can still call postprocess
        # depth_img = model.postprocess(shape_info, depth_res.copy())

        # Return the raw depth data (in meters)
        return depth_res
